...
1. Someone labels an issue `ready`. Its body is the plan. The worker only takes issues whose author is in the organization or a repository collaborator, whatever the repository's visibility; others are skipped and logged.
...
and uploads the recording to the public evidence bucket (`bun run pr evidence`), next to the installers, with links that play in the browser. Finally it asks
