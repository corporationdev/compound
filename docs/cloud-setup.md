...
Releases are published to the public `corporationdev/compound-releases` repository, which the app's updater and the download links follow (see [releases.md](releases.md)). Upstream install telemetry, hard-coded Sentry reporting, and the upstream Homebrew tap workflow were removed; the updater was rebuilt on the public releases feed. Desktop builds are named Compound, with bundle ID `dev.corporation.compound`, Compound artwork, and `Compound-arm64.dmg` releases. The CLI command is now `compound`; update older scripts that invoke `dapi`. See [branding-audit.md](branding-audit.md).
