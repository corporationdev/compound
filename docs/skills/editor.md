...
- For music and sound effects, use the library tools: `library_search --kind music` or `--kind sfx` **without a query** first, to read every description, then `library_import` the chosen source and place the returned path in JSX. See [library.md](../reference/library.md).
- `context` reports which folder the app actually has open and where the playhead sits. In-app AI generation is unavailable in Compound: bring media in from the library or from disk instead of declaring `generate.*`.
...
- When the brief, project, or user specifies branding, follow it. Only when none is specified, fall back to the [Compound brand](../brand/README.md) — its design, voice, video, and library references, and the components and compositions bundled with them.
...
- For tasks that don't need an editing UI, keep the app in the background: from a shell, `compound open -b <dir>` launches it that way. Over MCP the app is already running, and `open` only opens the folder.
...
- [Library — finding, inspecting and importing music and sound effects](../reference/library.md)
...
- [Compound brand — the fallback when no other branding is specified](../brand/README.md)
