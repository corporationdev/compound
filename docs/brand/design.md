...
Use these hex values in Compound source.
...
Compound has **no brand accent colour**. The mark is monochrome, and the app icon is the mark on
white. Do not introduce one: build structure from scale, weight, spacing and luminance instead.

`#008CFF` is the product blue, the app's `--primary`. Use it only when it already appears in real
product UI. Never use it as a brand accent. The app's destructive state is `#E62D2D`, so red
beside product UI reads as an error.

Place text on `#000000` or `#161616`. Do not rely on a shadow or stroke for contrast.

## Typography

Use `Inter`. Use `JetBrains Mono` for code and figures. Do not use italic. Both are the app's own
UI faces, so they match anything captured from the product.
...
Rewrite copy that exceeds a limit. Do not shrink the type. Confirm with `fonts` that Inter and
JetBrains Mono are registered on the machine before final output; no font files are bundled in
`assets/fonts/`.

## Logo and icon

Use `assets/logos/icon-white.svg` on a dark, quiet background. It is the Compound mark; there is
no wordmark file, so set the product name in type when the name must be clear.

Do not tint, outline, stretch, crop, rotate, rebuild, or place the mark over busy footage.
No wordmark, dark mark, minimum size, clear-space rule, or video-scale logo bug has been
...
The brand has no bundled fonts, wordmark SVG, dark logo, approved video logo bug, image library,
shared code text size, or measured minimum logo size. Name the gap when a task depends on one.
