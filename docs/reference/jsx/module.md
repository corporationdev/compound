...
| `.compound/jsx/` | App-owned copy of the `@compound/jsx` authoring types. The scaffold depends on `file:.compound/jsx`, so no published npm package is required. Read it, never edit it. |

Everything but `.compound/` is written once and is yours from then on. The reference itself is not copied into the project: it ships with the app, and the MCP server's instructions give agents the path of the installed version's `docs/`.
