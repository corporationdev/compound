...
`active`, `selected`, `timeline` and `playhead` are editor concerns rather than part of the composition. The file reads as the project's **opening state**: the scene that is active, what is selected, where the timeline is scrolled and zoomed, and where the playhead stands when the project is opened. From there the editor keeps its live view — playhead, selection, camera, timeline, expanded rows — locally, per project and per machine, and no longer writes any of it back to the file: a scrub, a click or a scroll changes nothing in the source. `x` and `y` are different: a scene dragged on the canvas moves in the file, since where a scene sits is part of the composition. There is no need to author a `timeline` or a `playhead`; the file's values only say where the project opens the first time.
