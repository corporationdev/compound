...
Both are editor state rather than composition: nothing rendered or exported depends on either. `camera` is read as the project's **opening view**; from there the editor keeps its live view (camera, playhead, selection, timeline, expanded rows) locally and no longer writes it back to the file, so a panned canvas leaves the source alone. Still give the stage a `camera`: a project that never says where to look opens framed on a 1920×1080 frame at the origin, roughly centered in a default window — so a single landscape scene at `x={0} y={0}` needs none. Write one when the composition is anything else: a portrait or unusual format, a scene placed away from the origin, or several scenes side by side.
