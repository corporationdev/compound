...
| --- | --- |
| MCP tool | `screenshot` |
| CLI | `compound screenshot [options]` |
...
The shot is taken from the app's own window buffer, so it works even when the window is hidden or covered by other windows, and never includes anything outside the app. The file is named `compound_<date>_<time>.png`; without an `output` directory it lands in the system temp directory.
