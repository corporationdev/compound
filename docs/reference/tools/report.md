...
| `commands` | `string[]` | `-c, --commands <cmd...>` | the compound commands or tool calls that reproduce it, in order |
| `logs` | `integer` | `--logs <n>` | trailing app log entries to attach (0 to omit; default: 50) |

For a tool that errors, contradicts this reference, or returns something it shouldn't. The description is bundled with diagnostics (app version, platform, Electron version, the app's recent console output) and filed as a GitHub issue on [corporationdev/compound](https://github.com/corporationdev/compound/issues).
...
`commands` are rendered as a shell block under `## Repro`, in the order given; from a shell, `-c` is repeatable (`-c "compound context" -c "compound capture intro"`). `logs: 0` omits the log section.
