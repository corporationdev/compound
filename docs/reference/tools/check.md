...
Finding issues is not a failure: the result is the same object either way, and the caller reads `issues`. From a shell, `compound check` additionally exits `1` when an error-severity issue is found (warnings alone stay `0`), so it can gate a script.
