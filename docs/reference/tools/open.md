...
| --- | --- |
| MCP tool | `open` |
| CLI | `compound open [path] [options]` |
...
- a folder with no project record gains one in `package.json` — `projectId`, `displayName`, `main`, and the compound commands as `scripts` — which is what the app remembers the folder by. A package.json that is already there keeps everything it has and only gains the fields it lacks.
