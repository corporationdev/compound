...
| --- | --- |
| MCP tool | `media_grab` |
| CLI | `compound media grab <path> [options]` |
| CLI aliases | `compound media sample` |
...
With `separate`, frames keep their own resolution and alpha, and each file is named after its timecode (e.g. `01s12f.png`). Without an `output` directory the images land in a fresh `compound-grab-*` directory under the system temp directory, so runs never overwrite each other. Writing into the same directory twice overwrites images whose name matches; with `separate`, requested times that land on the same frame share one file.
