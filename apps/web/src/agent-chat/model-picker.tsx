...
import { ensureConnected, harnesses as probeHarnesses, modelLabel, refreshHarnesses, type HarnessId, type HarnessInfo, type ModelRef } from "./store";

/** The icon for each agent Compound drives. */
export const harnessIcon = (harness: HarnessId | null | undefined): string =>
  harness === "claudeAgent" ? "claude-code" : harness === "codex" ? "codex" : "fx";
...
export function ModelPicker(props: ModelPickerProps) {
  ensureConnected();
  const harnesses = () => probeHarnesses();
            ...
            <DropdownMenuGroup>
              <DropdownMenuItem disabled>
                <span class="min-w-0 flex-1 truncate text-muted-foreground">
                  {window.desktop ? "Connecting…" : "Chat runs in the desktop app"}
                </span>
              </DropdownMenuItem>
            </DropdownMenuGroup>
          </Show>
