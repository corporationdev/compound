...
import { ChatAsset, Markdown } from "./markdown";
import "./activity.css";

type Of<K extends Item["kind"]> = Extract<Item, { kind: K }>;

export function UserItem(props: { item: Of<"user">; threadId: string }) {
  return (
    <div class="rounded-md bg-muted px-2 py-1.5">
      ...
      {/* Images the user pasted or picked: uploaded to T3, shown from its signed URL. */}
      <For each={props.item.assets}>
        {(asset) => (
          <ChatAsset
...
export function AssistantItem(props: { item: Of<"assistant">; threadId: string }) {
  return (
    <div class="px-0.5">
      <Markdown text={props.item.text} threadId={props.threadId} />
    </div>
  );
}
  ...
  return (
    <div class="pr-0.5">
      ...
      >
        ...
        {/* The turn ended before this call reported back: neither done nor failed. */}
        <Show when={props.item.status === "stopped"}>
          <Icon name="dot" class="size-6 shrink-0" />
        </Show>
...
/** Dispatches on kind. */
export function ChatItem(props: { item: Item; threadId: string }) {
  const item = () => props.item;
  return (
    <>
      <Show when={item().kind === "user"}>
        <UserItem item={item() as Of<"user">} threadId={props.threadId} />
      </Show>
      <Show when={item().kind === "assistant"}>
        <AssistantItem item={item() as Of<"assistant">} threadId={props.threadId} />
      </Show>
