 ...
 * from the picker — any folder will do, `resolveTarget` makes it a Compound
 * project — or, until either is chosen, a fresh project under the
 * application folder.
 */
type PromptTarget =
  | { kind: "new" }
...
export function DashboardHomeView() {
  ...
  // the picker is fed by T3's provider probes, so what it offers is what is
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // The chat starts before the page switches: T3 has the turn as soon as
      // it answers, and the editor lands with the reply streaming.
