...
// `compound mcp`: the entry point for agents that only run stdio servers (Claude
...
export async function runProxy(): Promise<void> {
  ...
  const fail = (error: Error): void => {
    console.error(`[compound mcp] ${error.message}`);
    process.exit(1);
  };
  ...
  upstream.onerror = (error) => console.error(`[compound mcp] ${error.message}`);
  stdio.onerror = (error) => console.error(`[compound mcp] ${error.message}`);
