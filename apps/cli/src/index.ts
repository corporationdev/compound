...
import { MCP_URL, toolByName } from "@compound/dapi";
import { APP_NAME, call, isAppDown, launchApp, ping, waitForApp } from "./cli-client";
import { runProxy } from "./mcp-proxy";

import type { GenericTool, ToolInput, ToolName } from "@compound/dapi";
...
program
  .command("projects")
  .description(`Projects ${APP_NAME} knows about`)
  .command("list")
  .description(describe("projects_list"))
  .action(() => run("projects_list", {}));
...
const media = program
  ...
  .alias("m")
  .description(
    "Inspect a media file by path or `library:<source-id>`, without adding it to the project: probe metadata, transcribe speech, grab frames, render visual previews, and analyze with multimodal models. Local files work with or without an open project; project-relative asset paths need one, and `library:<source-id>` needs only the signed-in app.",
  );
...
const library = program
  .command("library")
  .description(
    "Find music and sound effects in your library and the shared catalog. For general selection, search without a query to browse every description; import a source ID, then place the returned local path in the project's JSX.",
  );

library
  .command("search")
  .description(describe("library_search"))
  .argument("[query]", field("library_search", "query"))
  .requiredOption("--kind <kind>", field("library_search", "kind"))
  .option("--expand", field("library_search", "expand"))
  .action((query: string | undefined, opts: Omit<ToolInput<"library_search">, "query">) =>
    run("library_search", { ...opts, query }),
  );

library
  .command("get")
  .description(describe("library_get"))
  .argument("<source-id>", field("library_get", "sourceId"))
  .action((sourceId: string) => run("library_get", { sourceId }));

library
  .command("resolve")
  .description(describe("library_resolve"))
  .argument("<url>", field("library_resolve", "url"))
  .action((url: string) => run("library_resolve", { url }));

library
  .command("import")
  .description(describe("library_import"))
  .argument("<source-id>", field("library_import", "sourceId"))
  .action((sourceId: string) => run("library_import", { sourceId }));
